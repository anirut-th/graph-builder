...
function CreateLegend(viewport, graph, option, legendData) {
    ...
    let header = [ "Name", option.x_axis.name, option.y_axis_left.name, option.y_axis_right.name];
    
    //create legends header
    // 30% width of first column
    // 70% share evenly for the rest column
    let firstColWidth = legendBbox.width * 0.30;
    let otherColWidth = (legendBbox.width * 0.70) / 3;
    ...
    for (let i in legendData) {
        ...
        legendItems.append("text")
            ...
            .attr("text-anchor", "end")
            .style("dominant-baseline", "hanging")
            .attr("class", "y-left-value")
            .text("-")
        legendItems.append("text")
            .attr("id", "not-export")
            .attr("x", (firstColWidth + (otherColWidth * 3)))
            .attr("y", 12)
            .attr("text-anchor", "end")
            .style("dominant-baseline", "hanging")
            .attr("class", "y-right-value")
            .text("-")
    }
